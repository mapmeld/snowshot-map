...
def generate_client_login_request_body(email, password, service, source, 
  ...
  """
  return gdata_gauth.generate_client_login_request_body(email, password,
...
def get_client_login_token(http_body):
  ...
  """
  return gdata_gauth.get_client_login_token_string(http_body)
...
def extract_client_login_token(http_body, scopes):
  ...
  Args:
    ...
    scopes: list containing atom_url.Urls or strs. The scopes list contains
...
def get_captcha_challenge(http_body, 
  ...
  """
  return gdata_gauth.get_captcha_challenge(http_body, captcha_base_url)
...
def GenerateOAuthRequestTokenUrl(
  ...
  Returns:
    atom_url.Url OAuth request token URL.
  """
  ...
  return atom_url.parse_url(oauth_request.to_url())
...
def GenerateOAuthAuthorizationUrl(
  ...
  Returns:
    atom_url.Url OAuth authorization URL.
  """
...
def GenerateOAuthAccessTokenUrl(
  ...
  Args:
    authorized_request_token: gdata_auth.OAuthToken OAuth authorized request
        token.
    oauth_input_params: OAuthInputParams OAuth input parameters.    
  ...
  Returns:
    atom_url.Url OAuth access token URL.
  """
...
def generate_auth_sub_url(next, scopes, secure=False, session=True,
  ...
  Args:
    next: atom_url.URL or string The URL user will be sent to after
    ...
    session: boolean (optional) Determines whether or not the issued token
             can be upgraded to a session token.
    request_url: atom_url.Url or str The beginning of the request URL. This
...
def extract_auth_sub_token_from_url(url, 
  ...
  Args:
    url: atom_url.Url or str representing the current URL. The token value
  ...
  def __init__(self, signature_method, consumer_key, consumer_secret=None,
    ...
    Args:
      signature_method: class which provides implementation for strategy class
          ...
          gdata_auth.OAuthSignatureMethod.HMAC_SHA1. Instead of passing in
...
class ClientLoginToken(atom_http_interface.GenericToken):
  ...
  Args:
    ...
    scopes: list of str or atom_url.Url specifying the beginnings of URLs
...
class OAuthToken(atom_http_interface.GenericToken):
  ...
  def GetAuthHeader(self, http_method, http_url, realm=''):
    ...
    Args:
      ...
      http_url: string or atom_url.Url HTTP URL to which request is made.
    ...
    if isinstance(http_url, types.StringTypes):
      http_url = atom_url.parse_url(http_url)
    header = None
    token = None
