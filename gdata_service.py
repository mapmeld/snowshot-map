...
"""GDataService provides CRUD ops. and programmatic login for gdataservices.
...
import atom_service as atomservice
import gdata_init as gdata
import atom_init as atom
import http_interface as atom_http_interface
import token_store as atom_token_store
import auth as gdata_auth
import gauth as gdata_gauth
...
http_request_handler = atomservice
...
class GDataService(atomservice.AtomService):
  ...
  def __init__(self, email=None, password=None, account_type='HOSTED_OR_GOOGLE',
    ...
    Args:
      ...
      http_client: An object responsible for making HTTP requests using a
          ...
          atom_http.ProxiedHttpClient will be used.
      token_store: Keeps a collection of authorization tokens which can be
          ...
          find_token based on a URL (atom_url.Url or a string), add_token,
          and remove_token.
    """
    atomservice.AtomService.__init__(self, http_client=http_client, 
  ...
  def SetOAuthInputParameters(self, signature_method, consumer_key,
    ...
    Args:
      signature_method: class which provides implementation for strategy class
          ...
          constants defined by gdata_auth.OAuthSignatureMethod. Currently
          they are gdata_auth.OAuthSignatureMethod.RSA_SHA1 and
          gdata_auth.OAuthSignatureMethod.HMAC_SHA1
  ...
  def FetchOAuthRequestToken(self, scopes=None, extra_parameters=None,
    ...
    Returns:
      The fetched request token as a gdata_auth.OAuthToken object.
    ...
    if oauth_callback:
      if extra_parameters is not None:
        ...
        extra_parameters = {'oauth_callback': oauth_callback}
    request_token_url = gdata_auth.GenerateOAuthRequestTokenUrl(
  ...
  def SetOAuthToken(self, oauth_token):
    ...
    Args:
      request_token: gdata_auth.OAuthToken OAuth request token.
    """
  ...
  def GenerateOAuthAuthorizationURL(
    ...
    Args:
      request_token: gdata_auth.OAuthToken (optional) OAuth request token.
          If not specified, then the current token will be used if it is of
          type <gdata_auth.OAuthToken>, else it is found by looking in the
    ...
    """
    if request_token and not isinstance(request_token, gdata_auth.OAuthToken):
      raise NonOAuthToken
    if not request_token:
      if isinstance(self.current_token, gdata_auth.OAuthToken):
        request_token = self.current_token
      else:
        current_scopes = lookup_scopes(self.service)
        if current_scopes:
          token = self.token_store.find_token(current_scopes[0])
          if isinstance(token, gdata_auth.OAuthToken):
            request_token = token
    if not request_token:
      raise NonOAuthToken
    return str(gdata_auth.GenerateOAuthAuthorizationUrl(
        request_token,
  ...
  def UpgradeToOAuthAccessToken(self, authorized_request_token=None,
    ...
    Args:
      authorized_request_token: gdata_auth.OAuthToken (optional) OAuth request
          token. If not specified, then the current token will be used if it is
          of type <gdata_auth.OAuthToken>, else it is found by looking in the
          token_store by looking for a token for the current scope.
    ...
    if (authorized_request_token and
        not isinstance(authorized_request_token, gdata_auth.OAuthToken)):
      raise NonOAuthToken
    ...
    if not authorized_request_token:
      raise NonOAuthToken
    access_token_url = gdata_auth.GenerateOAuthAccessTokenUrl(
    ...
    if response.status == 200:
      token = gdata_auth.OAuthTokenFromHttpBody(response.read())
  ...
  def RevokeOAuthToken(self, request_url='%s/accounts/AuthSubRevokeToken' % \
    ...
    if not isinstance(token, gdata_auth.OAuthToken):
      raise NonOAuthToken
  ...
  def GetAuthSubToken(self):
    ...
    """
    if isinstance(self.current_token, gdata_auth.AuthSubToken):
      return self.current_token.get_token_string()
    current_scopes = lookup_scopes(self.service)
    if current_scopes:
      token = self.token_store.find_token(current_scopes[0])
      if isinstance(token, gdata_auth.AuthSubToken):
        return token.get_token_string()
    else:
      token = self.token_store.find_token(atom_token_store.SCOPE_ALL)
      if isinstance(token, gdata_auth.ClientLoginToken):
        return token.get_token_string()
      return None
  ...
  def SetAuthSubToken(self, token, scopes=None, rsa_key=None):
    ...
    Args:
     token: gdata_auth.AuthSubToken or gdata_auth.SecureAuthSubToken or string
    ...
    """
    if not isinstance(token, gdata_auth.AuthSubToken):
      token_string = token
      if rsa_key:
        token = gdata_auth.SecureAuthSubToken(rsa_key)
      else:
        token = gdata_auth.AuthSubToken()
  ...
  def GetClientLoginToken(self):
    ...
    """
    if isinstance(self.current_token, gdata_auth.ClientLoginToken):
      return self.current_token.get_token_string()
    current_scopes = lookup_scopes(self.service)
    if current_scopes:
      token = self.token_store.find_token(current_scopes[0])
  ...
  def SetClientLoginToken(self, token, scopes=None):
    ...
    """
    if not isinstance(token, gdata_auth.ClientLoginToken):
      token_string = token
      token = gdata_auth.ClientLoginToken()
      token.set_token_string(token_string)
  ...
  def ProgrammaticLogin(self, captcha_token=None, captcha_response=None):
    ...
    requests to gdataservices. The auth token is stored in the GData client
    object.
    ...
    """
    request_body = gdata_auth.generate_client_login_request_body(self.email,
    ...
    if auth_response.status == 200:
      ...
      self.SetClientLoginToken(
          gdata_auth.get_client_login_token(response_body))
      self.__captcha_token = None
      self.__captcha_url = None
      ...
      captcha_parameters = gdata_auth.get_captcha_challenge(response_body,
  ...
  def GenerateAuthSubURL(self, next, scope, secure=False, session=True, 
    ...
    if not isinstance(scope, (list, tuple)):
      scope = (scope,)
    return gdata_auth.generate_auth_sub_url(next, scope, secure=secure, 
        session=session, 
  ...
  def UpgradeToSessionToken(self, token=None):
    ...
    Args:
      token: A gdata_auth.AuthSubToken or gdata_auth.SecureAuthSubToken
  ...
  def upgrade_to_session_token(self, token):
    ...
    Returns:
      The upgraded token as a gdata_auth.AuthSubToken object.
    ...
    if response.status == 200:
      token.set_token_string(
          gdata_auth.token_from_http_body(response_body))
      return token
    else:
      raise TokenUpgradeFailed({'status': response.status,
                                'reason': 'Non 200 response on upgrade',
  ...
  def GetEntry(self, uri, extra_headers=None):
    ...
    See also documentation for gdataservice.Get
  ...
  def Post(self, data, uri, extra_headers=None, url_params=None,
           ...
           converter=None):
    """Insert or update  data into a gdataservice at the given URI.
  ...
  def PostOrPut(self, verb, data, uri, extra_headers=None, url_params=None, 
           escape_params=True, redirects_remaining=4, media_source=None, 
           converter=None):
    """Insert data into a gdataservice at the given URI.
...
def ExtractToken(url, scopes_included_in_next=True):
  ...
  token = gdata_auth.AuthSubTokenFromUrl(parsed[4])
  scopes = ''
...
def GenerateAuthSubRequestUrl(next, scopes, hd='default', secure=False,
  ...
  if include_scopes_in_next:
    if next.find('?') > -1:
      ...
      next += '?%s' % urllib.urlencode({SCOPE_URL_PARAM_NAME:scope})
  return gdata_auth.GenerateAuthSubUrl(next=next, scope=scope, secure=secure,
...
class Query(dict):
  ...
  def ToUri(self):
    ...
    return atomservice.BuildUri(q_feed, self)
